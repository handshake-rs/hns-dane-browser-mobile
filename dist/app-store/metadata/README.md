...
is `0.5.5` (build `57`). Build `48` is retained as predecessor upload evidence
and build `49` as a superseded `0.5.5` upload. Builds `50`–`56` were not
uploaded. Build `56` passed exact CI, but its live screenshot run exposed
indeterminate dual-root ICANN negative evidence caused by compressed SOA names
and a stale plan-start freshness timestamp. Build `57` contains the shared
parser and post-resolution timing fix; its gates and upload remain pending.
...
6. Complete every item in `../submission-checklist.md`, select build `57`, and choose manual release before adding the version for review.
