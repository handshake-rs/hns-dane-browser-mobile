...
final class BrowserViewController: UIViewController {
    ...
    private func presentLegal() {
        ...
        let agreement = """
        This is an experimental Handshake-first browser with local HNS proofs, authoritative DNS, an optional requester-only P2P DNS relay, proof-anchored authoritative DoH, an optional user-configured recovery DoH endpoint, DNSSEC/DANE diagnostics, and a device-local HNS wallet. The wallet's direct mode can receive and send HNS, manage names, and use peer-backed marketplace workflows through wallet-owned verified Handshake peers. Direct swap discovery makes a best-effort public IPv6 or PCP/NAT-PMP/UPnP TCP reachability attempt; firewalls, carrier CGNAT, and unavailable router services can still prevent inbound connections. Complete atomic Shakedex settlement, website-provider wallet access, and HNSA/HNSR service roles remain unavailable or under development. HNS resolution, validation, relay, recovery, sync, transaction broadcast, name operations, marketplace workflows, and wallet storage may fail closed, be delayed, require later manual actions, or remain incomplete. Review every native approval, recipient, amount, fee limit, name, listing, and session before approving it. HNS WebPKI fallback is prohibited; recovery answers never bypass local DNSSEC, TLSA, or DANE validation. The requester-only P2P DNS relay is separate from HNSR and does not make this device an endpoint or output node. The app is provided without warranty and is not a financial service.
        """
