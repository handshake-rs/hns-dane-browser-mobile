...
final class BrowserViewController: UIViewController {
    ...
    private func presentLegal() {
        ...
        let agreement = """
        This is an experimental Handshake-first browser with local HNS proofs, authoritative DNS, an optional requester-only P2P DNS relay, proof-anchored authoritative DoH, an optional user-configured recovery DoH endpoint, DNSSEC/DANE diagnostics, and native controls for one device-local non-value HNS account identity. HNS resolution, validation, relay, recovery, sync, and wallet storage may fail closed or be incomplete. HNS WebPKI fallback is prohibited; recovery answers never bypass local DNSSEC, TLSA, or DANE validation. Wallet balances, transfers, names, website-provider access, HNSA/HNSR service roles, settlement, exchange features, and P2P marketplaces are unavailable. The requester-only P2P DNS relay is separate from HNSR and does not make this device an endpoint or output node. The app is provided without warranty and is not a financial service.
        """
