...
- Added Android and iOS direct Shakescape V1 listeners, explicit IP-literal peer
...
- Switched the native wallet closure from the stale published `0.2.0` cohort
  ...
  `bd16ce1d33bc620ccddede8636f411892188d2f4`, which refreshes all persisted
  and locally discovered FINALIZE name proofs before the final wallet snapshot
  without rewinding the authenticated transaction scan.
...
- Renamed the public Android and iOS product to Shakescape and applied the new
  icon, launch, start-page, store, and screenshot visual system while retaining
  the existing application IDs, bundle IDs, source package names, and data.
