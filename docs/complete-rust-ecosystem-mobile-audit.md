...
workspace layout. The umbrella facade and its HNSA/HNSR protocol types remain
an upstream mobile-portability prerequisite and are not in this graph.
...
engine adapters and canonical contracts from one exact qualified engine `0.2.0`
Git revision; standalone facade adoption remains pending.
