...
| Native wallet lifecycle | Feature source qualified; exact release gates pending | Android JNI and the Apple C ABI currently link `hns-wallet-mobile` from intermediate wallet release-preparation source `f83d42363305de04bfa955f864cb1e9136c4d648` to native create, restore, open, status, unlock, lock, one-time recovery, destroy, and exactly one non-value HNS account identity. Android uses create-only KeyStore wrapping and a mutable non-copyable recovery view; iOS uses a create-only ThisDeviceOnly/user-presence Keychain item and best-effort UIKit text clearing. Provider/WebView access, balances, transfers, names, sending, settlement, exchange, HNSA/HNSR controls, Shakedex/Denuo, and P2P marketplaces remain unavailable. Fresh-install Android qualification and the complete Apple gate passed on the pre-repin exact feature source. After the dated final `hns-rs` commit, wallet and mobile must repin in sequence; that final `0.5.8` source then needs exact-head CI, signed artifacts, current screenshots, and store declaration review. |
