...
pinned `hns-dane-engine 0.2.0` source
`0346c25973d5c91de94d49b47349ec9b9efce3a0`; the former compatibility patch
bridge is gone and a standalone checkout no longer depends on the coordination
workspace layout. This ECH-capable pin requires fresh exact-source platform
qualification. The umbrella facade and its HNSA/HNSR protocol types remain an
upstream mobile-portability prerequisite and are not in this graph.
...
engine adapters and canonical contracts from one exact pinned engine `0.2.0`
Git revision; the ECH-capable pin requires fresh qualification, and standalone
