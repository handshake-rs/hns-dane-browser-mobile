...
| Native wallet lifecycle, reads, and exact-name import | Published dependency cohort; product release gates open | Android JNI and the Apple C ABI link published `hns-wallet-mobile 0.2.0`, whose HNS closure is published `hns-rs 0.3.1`, to native lifecycle, value, name, Bitcoin, and Swap Board controls. The exact registry/checksum and registry-only browser-adapter policy is recorded in [released-dependency-cohort.md](released-dependency-cohort.md). Exact-name import preserves UTF-8 bytes, returns one minimized summary, refreshes rows after success, treats invalid input as non-poisoning, and locks on runtime faults. Signed artifacts, current screenshots, store declaration/upload, credentialed Android qualification, and the physical-iPhone matrix remain open. |
