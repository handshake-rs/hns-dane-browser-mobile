...
source coordinates Android `1.0.4` code `56`, embedded Rust `1.0.0`, and the
...
| Native wallet lifecycle, reads, and exact-name import | Published dependency cohort; product release gates open | Android JNI and the Apple C ABI link `hns-wallet-mobile 0.2.1` from its exact published source revision; its HNS closure is published `hns-rs 0.4.1` and its direct light-client closure is the coherent engine `0.2.3` patch cohort. The exact source/checksum policy is recorded in [released-dependency-cohort.md](released-dependency-cohort.md). Exact-name import preserves UTF-8 bytes, returns one minimized summary, refreshes rows after success, treats invalid input as non-poisoning, and locks on runtime faults. Signed artifacts, current screenshots, store declaration/upload, credentialed Android qualification, and the physical-iPhone matrix remain open. |
