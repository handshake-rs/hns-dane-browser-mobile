...
`49`, embedded Rust `0.5.8`, iOS build `58`). Its underlying wallet tranche
passed exact-source Android and Apple qualification at `571ea0c`, but the final
repin/version commit still requires exact-head CI and signed/store gates. This
checkpoint is not evidence that a public store build ships wallet controls.
...
| Native wallet lifecycle | Final dependency source configured; exact release gates pending | Android JNI and the Apple C ABI link `hns-wallet-mobile` from final wallet `0.1.0` source `4e78bb2587bc448d3a65341c7628b2e62cae79cd`, with final `hns-rs 0.2.0` closure `b24b66c382de53330ec21dd3137e056a2bea3e2d`, to native create, restore, open, status, unlock, lock, one-time recovery, destroy, and exactly one non-value HNS account identity. Android uses create-only KeyStore wrapping and a mutable non-copyable recovery view; iOS uses a create-only ThisDeviceOnly/user-presence Keychain item and best-effort UIKit text clearing. Provider/WebView access, balances, transfers, names, sending, settlement, exchange, HNSA/HNSR controls, Shakedex/Denuo, and P2P marketplaces remain unavailable. Fresh-install Android qualification and the complete Apple gate passed on the pre-repin exact feature source. The final dependency sequence is complete; the resulting `0.5.8` source now needs exact-head CI, signed artifacts, current screenshots, and store declaration review. |
...
| Foreground/background and browser restart qualification | Partial | Existing lifecycle and proxy-revocation tests remain. Targeted exact-signed code 47 checks passed on a Pixel 9 for upgrade with preserved data, cold launch, manual sync, HNS browsing, and corrected HNS/ICANN Proof Details presentation. The exact wallet-feature APK then passed fresh reinstall, create/confirm, unlock/lock, process reopen, private file-mode, and mainnet/testnet isolation checks; Required CI `31393998309` passed the complete Apple gate. The broader mobile-network, requester/recovery, Service Worker, download, WebSocket, cross-origin, Android restore/background, and real-iPhone matrix remains open. The iOS physical-device matrix is not an App Store submission prerequisite but remains an installed-device ecosystem limitation. The PDF's unimplemented ODoH/HNSR lifecycle cases remain future work. |
