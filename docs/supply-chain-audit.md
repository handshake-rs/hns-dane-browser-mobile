...
### Current `0.5.1` Candidate

- Android declares `0.5.1` / code 41, the shared Rust workspace declares `0.5.1`, and iOS declares `0.5.1` / build 45. This is not a metadata-only release: it changes shared namespace classification, resolver interception handling, header currentness, and maintenance/publication coordination.
- The five engine crates are pinned to reviewed immutable `handshake-rs/hns-dane-engine` commit `be58b041b74403042b134a95dcc4588c89ad549b`.
- The complete portable `scripts/check.sh` gate passed on 2026-07-26 against the candidate source: supply-chain, generated-notice, version, runtime-boundary, formatting, warning-denied Clippy, Apple C ABI, cargo-deny, full workspace tests, fuzz smoke, and header-snapshot-exporter checks all completed successfully.
- Android Gradle build/lint and artifact gates remain blocked on this Linux host until the required Android SDK/NDK is restored; Apple XCTest/archive validation requires macOS/Xcode.
- Hosted path-policy, Rust, cold-cache Android, Apple, and required-result jobs are pending for the exact candidate commit.
- No signed `0.5.1` APK/AAB exists yet. Release signing, bundle verification, device acceptance, and Play upload remain intentional external gates.
...
- Those artifacts predate the current source and do not validate or identify the `0.5.1` candidate.
