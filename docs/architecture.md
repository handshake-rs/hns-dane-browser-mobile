...
- External engine contracts: `hns-browser-runtime` owns the canonical session, generation, lifecycle, and admitted-work stamps; `hns-browser-observability` checks the name-free schema-v2 browser status; `hns-icann-dane` derives transport-aware TLSA owners and constrained ICANN DANE/WebPKI outcomes; `hns-namespace-resolution` validates complete single-root origin plans, compares them, applies explicit-pin/sticky/ICANN precedence, fingerprints the selected immutable decision, and retains bounded per-root present/absent/failed dispositions when classification fails; `hns-resolution-policy` provides the typed direct-first transport plan. Mobile maps its stable relay ABI and normalized explicit recursive-recovery choice into that policy with ODoH, HNSR, provider roles, and legacy compatibility explicitly disabled. Unchanged browser adapters remain at published `0.2.2`/`0.2.3`, `hns-browser-observability` and `hns-resolution-policy` use the clean-break published `0.3.0` policy graph, the synchronized light-client/SQLite adapters use `0.2.5`, and the wallet closure uses the exact checksum-bearing published `0.2.6` cohort. The [released dependency cohort](released-dependency-cohort.md) records the boundary.
- The upstream `hns-dane-engine` facade contains the reviewed HNSA admission and HNSR requester types but is deliberately absent from this mobile graph because it currently pulls public OpenSSL-backed DANE/DNSSEC crates into Android and Apple closures. The platform shells define only a dormant one-shot `hns.named-service/v1` wallet-consumer seam: it accepts an opaque broker-issued result, binds its exact HRM root/revision/trusted-time/fenced-lease and service authority to the live wallet, and keeps the guard held through dependent use. Kotlin and Swift do not parse or verify HRM/HNSA. A mobile-safe upstream boundary must land before this seam can be populated or HNSR can be adopted. This checkout does not construct those runtimes or provide their proof, rollback-resistant persistence, Brontide/Denuo transport, endpoint, rendezvous, provider, FFI, or native-control adapters.
- The compatibility names `hns-core`, `hns-chain`, `hns-p2p`, `hns-sync`,
  `hns-urkel`, `hns-resolver`, `hns-dnssec`, `hns-dane`, `hns-transport`,
  `hns-gateway`, and `hns-loopback-proxy` are Cargo aliases for their published
  `hns-browser-*` adapter packages at exact `0.2.2`. They must not be restored
  as local product crates.
...
- `hns-transport`: the engine-owned mobile transport backend provides bounded HTTP/1.1, HTTP/2, and HTTP/3 origin transport with pooling/resumption partitioned by selected namespace fingerprint and validated-head-before-body streaming. Selected-plan HTTPS/SVCB ECHConfigList bytes enable TLS 1.3 ECH for those protocols and secure WebSocket Upgrade; ECH acceptance and DANE/WebPKI policy complete before application bytes, and rejection is terminal rather than a plaintext retry. ECH configuration also partitions verifier, resumption, and HTTP/1.1 pool identity and disables unrelated Alt-Svc promotion. A fingerprinted retained plan cannot otherwise be mutated by Alt-Svc into another protocol, transport, endpoint, or TLSA owner. Legacy stateless-DANE parsing remains research code, but prepared gateway requests reject that mode because it cannot be represented without a second authority.
...
- `hns-mobile-platform-runtime`: mobile platform adapter and ownership boundary for storage, synchronization, sockets, proxy lifecycle, and Android/Apple shell integration. It independently builds HNS and ICANN plans for every complete DNS host, queries both A and AAAA when its authenticated root-label state permits the name to exist, retains DNSSEC denial/signature expiry and typed root failures, compares full endpoints and service policy, applies explicit pin then sticky selection then ICANN default, and persists HTTPS/WSS and HTTP/WS under shared binding keys. Cold HNS and ICANN root-label probes run concurrently. A bounded HNS cache is keyed by name hash under one exact authoritative Urkel tree root and is cleared as a unit when that tree root changes. A separate bounded ICANN cache can suppress every descendant lookup only after a secure NXDOMAIN response to an explicit TLD NS query, and expires at the lesser authenticated DNS TTL/RRSIG deadline already retained by the evidence layer. Errors, NODATA, malformed responses, and insecure negatives never populate either absence path. A pinned hsd consensus snapshot permits a still narrower mainnet optimization after claim expiry and ancestry through the bundled post-ICANNLOCKUP checkpoint: exact verified non-inclusion of a root-flagged ICANN name learns an in-process tombstone, while inclusion always takes the ordinary conflict path. HNS resolution follows the shared direct-first order: authoritative UDP/TCP, authenticated authoritative DoH, the policy-admitted P2P requester only under independent opt-in, then separately configured recursive HNS DoH only after transport failure or confirmed interception. The recovery hostname is resolved only through validating ICANN DoH, addresses must be public, endpoint TLS is WebPKI, and answers re-enter the same local DNSSEC/TLSA/DANE validator. DNS response codes, invalid DNS, bogus DNSSEC, and stale or missing HNS proof state do not authorize recovery. HTTPS/SVCB records are parsed as one RRset and filtered for mandatory parameters, usable ALPN, port, and local ECH support before priority selection. A compatible ECHConfigList is retained exactly; valid unsupported optional ECH is omitted, mandatory unsupported ECH makes that record ineligible, and malformed known parameter encoding rejects the RRset. Browser-wide GREASE remains future policy because rustls' GREASE mode forces TLS 1.3; applying it only to optional unsupported ECH would create a case-specific compatibility cliff. The current single-plan model chooses a canonical member of a same-priority set so independently built root plans converge; retaining and randomly shuffling connection alternatives remains future plan-model work. Protocol candidates from that record are tried as HTTP/3, HTTP/2, then RFC 9460 implicit-or-explicit HTTP/1.1; HNS advances only on secure TLSA absence for that candidate's UDP/TCP owner, while insecure or bogus evidence is terminal. ICANN DoH authenticates its configured hostname while connecting only to pinned bootstrap addresses. The selected plan, trace, and error namespace are retained together per request even when another request overwrites the shared plan cache; no host-system DNS or post-selection resolver call is permitted. The adapter drives the canonical `hns-browser-runtime` state machine with the same raw session identity as the stable proxy token, maps mobile policy revisions one-for-one to canonical generations, and emits checked `hns-browser-observability` success or failure status only when exact typed evidence is representable. Bogus and indeterminate DNSSEC are never collapsed into namespace or TLSA absence, and generic transport/WebPKI failures are unavailable rather than relabeled as DANE or SNI failures.
...
- `MainActivity`: single-WebView browser shell with custom omnibox, left-side security status, a tab-count switcher, live-polled first-page sync progress bar and target stats, a separate WebView loading bar, hamburger-menu forward/refresh/home/new-tab actions, Service Worker native routing, and navigation controls. Its bounded `BrowserTabs` collection retains only each inactive tab's current URL and title—never a hidden WebView, JavaScript context, proxy lease, or background request—and clears shared WebView history at tab boundaries. Initial, omnibox, tab, history, reload, intent, and main-frame-link loads all pass through the proxy admission gate; every canonical DNS request reaches the shared loopback proxy, and completed loads consume selected-namespace status from the exact live proxy/host binding.
