...
- External engine contracts: `hns-browser-runtime` owns the canonical session, generation, lifecycle, and admitted-work stamps; `hns-browser-observability` checks the name-free schema-v2 browser status; `hns-icann-dane` derives transport-aware TLSA owners and constrained ICANN DANE/WebPKI outcomes; `hns-namespace-resolution` validates complete single-root origin plans, compares them, applies explicit-pin/sticky/ICANN precedence, fingerprints the selected immutable decision, and retains bounded per-root present/absent/failed dispositions when classification fails; `hns-resolution-policy` provides the typed direct-first transport plan. Mobile maps its stable relay ABI and normalized explicit recursive-recovery choice into that policy with ODoH, HNSR, provider roles, and legacy compatibility explicitly disabled. All five contracts use version `0.2.1` from exact pinned Git source `65c397e8347f37085ea67d2c9c745ce896328e64`; there is no crates.io patch bridge. Exact HNWR-v2 code-bearing source `986accb7d86d220af63187031e629a9ce69d71e5` passed the Rust, Android, and complete Apple platform gates with this ECH-and-sync-telemetry revision in full CI `31807520618`.
