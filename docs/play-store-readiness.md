...
Current pending Android candidate source is `1.0.7` (`versionCode 59`) and
...
older scoped-loopback compatibility seam. It exposes native Bitcoin, supported
name-operation, signed ShakeScape offer, and noncustodial cross-chain swap
controls; each value action remains gated by exact synchronization, peer,
balance, fee-reserve, and native-approval prerequisites. Website-provider and
HNSA/HNSR service roles remain unavailable.
...
| Native wallet candidate | HNS, Bitcoin, names, and direct ShakeScape active; exact application qualification pending | Source pins the reviewed, checksum-bearing wallet `0.2.5` registry cohort. The non-exported native wallet supports direct HNS and Bitcoin synchronization, receive/QR, guarded sends, activity, name actions, restoration birthday heights, signed offers, durable noncustodial BTC/HNS swap execution, and deletion. Website-provider and HNSA/HNSR service roles remain closed. Play signing, final two-device completion, and Console review/upload remain open. |
