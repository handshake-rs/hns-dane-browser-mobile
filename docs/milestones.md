...
- Added authenticated whole-browser proxy routing with an optional immutable HNS scope. DNS-named ICANN HTTPS/WSS CONNECT is locally terminated and uses validating DoH, automatic transport-aware TLSA discovery, DANE when secure TLSA is present, and WebPKI only for authenticated absence or insecure delegation. Public IP literals retain bounded opaque CONNECT. HTTP forwarding, Upgrade tunneling, and special-address/unsafe-port policy do not use system target resolution.
