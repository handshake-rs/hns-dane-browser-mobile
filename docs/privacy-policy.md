...
Shakescape is published by Denuo Web, LLC. For privacy questions or deletion requests, email <info@shakescape.com> or use the developer contact listed in the app's store listing. Do not post personal information to the public project issue tracker.

## Summary

Shakescape is a dual-root browser for local HNS proofs, authoritative DNS, optional requester-only HNS P2P DNS relay consumption, optional user-configured recursive HNS DoH recovery, DNSSEC, and DANE diagnostics. It also provides native device-local, noncustodial Handshake and Bitcoin wallet controls. These include create, restore, unlock, direct peer synchronization, receive, guarded send, recent activity, tracked names, supported Handshake name actions, restoration birthday heights, direct ShakeScape offer exchange, noncustodial BTC/HNS atomic swaps, and protected deletion. Every value-changing action remains inside native UI and requires explicit review and approval; websites cannot invoke wallet operations or read wallet state. The app does not take custody, match orders as an exchange, or recover wallet secrets. Its ShakeScape board peers relay signed offers and protocol messages between participants; settlement is enforced by participant-controlled on-chain transactions. Website-provider and HNSA/HNSR service-role screens remain unavailable. The requester-only P2P DNS relay is separate from HNSR and does not make the device a DNS relay endpoint or output node. The app has no advertising SDKs, analytics SDKs, developer-operated accounts, custodial exchange service, or paid feature unlocks. The Android edition may show an optional external donation link that does not unlock functionality; neither platform contains an in-app purchase flow.
...
- Native wallet data: a network-scoped encrypted wallet database containing the HNS account identity, Bitcoin wallet descriptors, public addresses and transaction records, tracked names, compact-filter and direct-peer synchronization checkpoints, pending-broadcast recovery state, locally created or received signed ShakeScape offers, durable atomic-swap execution journals, public peer endpoints, and the key material needed to reopen the wallet. Android keeps the database under app-private no-backup storage and wraps its 32-byte database key with Android Keystore. iOS uses an app-private, backup-excluded database with complete file protection and a ThisDeviceOnly Keychain item requiring user presence. A newly generated recovery phrase is shown once for offline backup; restore input and the one-time display are cleared when the wallet screen leaves its protected lifecycle. If the screen closes before that recovery display is confirmed, the app wipes its unconfirmed database-key buffer and deletes the incomplete wallet database instead of retaining an unrecoverable wallet. Swift/UIKit-managed text on iOS cannot be claimed to be deterministically zeroized, although app-owned mutable buffers are wiped.
- QR scanning: camera access is requested only after you tap the scanner in the native HNS send form. Camera frames and the decoded Handshake payment address are processed on the device and are not uploaded by Shakescape.
...
- Handshake peers and DNS seed hosts for browser header sync, wallet synchronization, peer discovery, proof retrieval, and broadcast of transaction bytes you explicitly approve.
- Bitcoin peers and Bitcoin peer-discovery sources for compact-filter synchronization, relevant-block retrieval, and broadcast of exact Bitcoin transaction bytes you explicitly approve.
- ShakeScape-capable Handshake peers and an endpoint you select or discover for signed offer exchange, atomic-swap negotiation, and authenticated settlement-status messages. A public rendezvous peer can observe your source IP address, connection timing, and the signed public protocol messages sent through that connection. It does not receive a recovery phrase or private key and cannot spend wallet funds.
...
The native wallet connects directly to Handshake and Bitcoin peers to synchronize public chain activity and to broadcast transaction bytes only after native review and explicit approval. When you pair with a ShakeScape peer, the app also exchanges signed public offers and bounded atomic-swap protocol messages needed to negotiate and monitor the swap. It does not contact a website wallet provider or send a recovery phrase, private key, encrypted wallet database, or unsigned spending authority to those peers.
...
Denuo Web does not sell personal or sensitive user data. Shakescape shares data only as necessary for user-requested browser and wallet functionality, such as loading a website, syncing public HNS or Bitcoin data, resolving a name, downloading a file, exchanging a signed public offer, negotiating a participant-to-participant atomic swap, or broadcasting an approved transaction. Native wallet databases, recovery phrases, private keys, and device-bound database keys are not sent to Denuo Web, websites, analytics services, or a wallet provider by this version. Public addresses and approved transactions are necessarily visible to participating peers and the relevant public blockchain. A ShakeScape counterparty or rendezvous peer receives the signed offer and swap-protocol fields required for the transaction, and the network connection exposes ordinary IP metadata.

## Noncustodial Wallet and Swap Functionality

Shakescape is software for users to control their own keys. Denuo Web does not hold funds, maintain exchange balances, choose a counterparty, guarantee an offer, or reverse a blockchain transaction. Direct BTC/HNS swaps use signed participant messages and on-chain hash-and-time-lock transactions. Each participant is responsible for reviewing the exact assets, amounts, fee reserves, deadlines, and settlement action shown by the app. Public blockchain fees and counterparty/network availability can affect completion. The app keeps a durable local execution journal so it can resume verification and expose an eligible redemption or refund after interruption; iOS may require the user to unlock and foreground the wallet before that work resumes.
