...
| Next-release wallet dashboard | Locked state exposes only Wallet; unlocked state exposes Names, Wallet, and Bitcoin; Shakedex card hidden | Locked state exposes only Wallet; unlocked state exposes Names, Wallet, and Bitcoin; Shakedex card hidden | Runtime Shakedex wiring remains present; the boundary check requires both immutable UI gates to stay equal and disabled | Re-enable only in a coordinated later Android/iOS release after marketplace qualification |
