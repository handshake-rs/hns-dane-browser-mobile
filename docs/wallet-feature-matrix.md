...
| Direct-peer synchronization | Foreground service retains user-started scans and active swap monitoring within Android's data-sync budget | Protected foreground coordinator automatically refreshes live-swap HNS and Bitcoin watches and durably resumes after iOS suspension | Verified peer/header agreement, rollback floor, closed progress bundles, current execution schema, and stale-publication tests | Final installed-device interruption/resume and iPhone network qualification |
...
| Payment QR ingress | Camera action beside Receive, Send, and Sync; scanner result resumes the guarded send flow | Camera action beside Receive, Send, and Sync; scanner result resumes the guarded send flow | Both shells accept only a parsed public Handshake payment URI and re-establish protected wallet authority before review | Final installed Android and physical-iPhone camera exercise |
