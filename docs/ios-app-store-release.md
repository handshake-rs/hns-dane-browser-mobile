...
- Current iOS release candidate: `0.5.1` (`45`)
...
4. verifies the archived app identity and compiled AppIcon catalog, then validates/exports the archive with App Store Connect authentication and uploads build `45`;
5. deletes the temporary keychain, installed profile, API key, `.p12`, and profile while GitHub discards the runner.

Apple associates the uploaded build with the app record using its bundle ID, version, and build number. A rerun after Apple accepts build `45` requires a higher build number.
