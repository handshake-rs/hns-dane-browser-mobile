...
- Previous submission: `1.0.8` (`71`), submitted with automatic release
  after approval on 2026-09-24
- Current candidate: `1.0.9` (`72`), automatic release after approval
- Device families: iPhone and iPad; compatible iOS-on-Apple-silicon-Mac use is
  permitted by the target

Candidate build `72` includes the current native-only wallet screen for
...
After the upload run succeeds and build `72` finishes processing, use the
...
gh workflow run ios-app-store-submit.yml \
  ...
  -f mode=submit \
  -f review_contact_source_version=1.0.4 \
  -f confirm_metadata=APPLY_METADATA_1.0.9_72 \
  -f confirm_submit=SUBMIT_FOR_REVIEW_1.0.9_72 \
...
gh workflow run ios-app-store-submit.yml \
  ...
  -f mode=auto-release \
  -f review_contact_source_version=1.0.4 \
  -f confirm_auto_release=SET_AUTO_RELEASE_1.0.9_72 \
  -f confirm_account_readiness=true
```
...
`-f confirm_screenshot_replacement=REPLACE_SCREENSHOTS_1.0.9_72`. Replacement
...
current `1.0.9` / build `72` candidate.
