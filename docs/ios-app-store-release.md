...
- Current iOS release candidate: `0.5.5` (`49`); build `48` is predecessor
  App Store Connect upload evidence
...
4. verifies the archived app identity and compiled AppIcon catalog, then
   ...
   authentication, uploads build `49`, and retains
...
version, and build number. A rerun after Apple accepts build `49` requires a
higher build number.

Build `49` declares `ITSAppUsesNonExemptEncryption = false` because the
