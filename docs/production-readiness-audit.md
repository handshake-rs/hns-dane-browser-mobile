...
non-publishable Rust `1.0.2`, and iOS `1.0.6` / build `68`. Both platform
...
| App Store update | `1.0.6` / build `68` prepared | The prior `1.0.5` / build `66` candidate is historical rather than pending. Current source, metadata, validation, and protected workflow guards target `1.0.6` / build `68`. Signing, exact-source Apple qualification, upload, metadata readback, and intentional submission remain open. The unapproved default-browser and MarketplaceKit app-installation entitlements remain absent. |
