...
`0.5.5`, and iOS `0.5.5` (build `56`). The matching `0.5.4` candidate passed
...
| Public App Store listing | New update build required; device qualification open | Apple reported version `0.5.0` on 2026-07-28. Build `48` is predecessor evidence and `0.5.5` / build `49` is a superseded upload. Builds `50`–`55` were not uploaded; their live-capture findings are incorporated into `0.5.5` / build `56`, the current update including `What's New`. No TestFlight distribution is part of this release. A real-iPhone pass remains a separate installed-iOS and ecosystem qualification item. |
