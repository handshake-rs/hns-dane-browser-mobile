...
`0.5.5`, and iOS `0.5.5` (build `53`). The matching `0.5.4` candidate passed
...
| Public App Store listing | New update build required; device qualification open | Apple reported version `0.5.0` on 2026-07-28. Builds `48` and `49` uploaded successfully; build `48` is predecessor evidence and `0.5.5` / build `49` is superseded upload evidence. Build `50` was not uploaded because clean simulator run 30414784116 reproduced the transient `-1005` alert; build `51` validation was canceled. Build `52` passed exact CI in run 30417946199, but live run 30417953393 reproduced two consecutive provisional `-1005` failures and its upload was canceled before credentials or signing. Treat `0.5.5` / build `53` as the current update, including `What's New`. A real-iPhone TestFlight pass is not a submission prerequisite, but remains required for installed-iOS and ecosystem qualification. |
