...
Its ECH-capable engine pin requires fresh exact-source CI and platform
...
| Area | Status | Finding |
| --- | --- | --- |
| `0.5.9` source identity | Versions aligned; ECH delta qualification and signed product pending | Android `0.5.9` / code `50`, embedded Rust `0.5.9`, and iOS `0.5.9` / build `59` are aligned. `hns-wallet-mobile` is pinned to final wallet `0.1.0` source `2229be849557d58a8eb723bcc03349f0f2df9796`, which uses final `hns-rs 0.2.0` source `b24b66c382de53330ec21dd3137e056a2bea3e2d`. Source policy, lockfile, and notices bind that chain. The pre-ECH source `893ba8271787f1ab7247fa78ed8787462b5542fc` passed full CI `31433931682`; the current ECH-capable source requires fresh qualification. Debug artifact `9080493058` is not upload/store signed, and no signed `0.5.9` candidate has been built or uploaded. |
...
| Build supply chain | Pre-ECH `0.5.9` CI green; ECH and signed-store verification pending | Required CI run `31402758394` and docs-parent run `31411048376` remain historical `0.5.8` evidence. Exact pre-ECH `0.5.9` source `893ba8271787f1ab7247fa78ed8787462b5542fc` passed policy, Rust/supply-chain, Android, Apple, and Required CI in `31433931682`; CodeQL `31433931259` and Code Quality `31433931278` also passed. The current ECH-capable source requires fresh CI and platform qualification. Upload signing and store artifact verification remain separate. |
...
3. Complete the remaining `0.5.9` product gates: provision and qualify a scoped
   ...
   HNWR-aware policy deployment, signed Android product qualification, and the
