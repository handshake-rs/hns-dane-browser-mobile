...
`0.5.5`, and iOS `0.5.5` (build `50`). The matching `0.5.4` candidate passed
...
| Public App Store listing | New update build required; device qualification open | Apple reported version `0.5.0` on 2026-07-28. Builds `48` and `49` uploaded successfully; build `48` is predecessor evidence and `0.5.5` / build `49` is superseded evidence from before the bounded navigation-recovery fix. Treat `0.5.5` / build `50` as the current update, including `What's New`. A real-iPhone TestFlight pass is not a submission prerequisite, but remains required for installed-iOS and ecosystem qualification. |
