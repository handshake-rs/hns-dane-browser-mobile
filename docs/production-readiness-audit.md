...
| Area | Status | Finding |
| --- | --- | --- |
| `1.0.0` source identity | Exact dependency cohort; signed candidate open | Android `1.0.0` / code `52`, iOS `1.0.0` / build `61`, and embedded Rust `1.0.0` are aligned. HNS, engine, and wallet dependencies are exact checksum-bearing published releases, including the complete wallet `0.2.1` cohort. [released-dependency-cohort.md](released-dependency-cohort.md) binds that policy. The final versioned source still requires complete CI, CodeQL, signing, screenshot, and store readback qualification. |
