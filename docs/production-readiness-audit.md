...
`0.5.5`, and iOS `0.5.5` (build `55`). The matching `0.5.4` candidate passed
...
| Public App Store listing | New update build required; device qualification open | Apple reported version `0.5.0` on 2026-07-28. Builds `48` and `49` uploaded successfully; build `48` is predecessor evidence and `0.5.5` / build `49` is superseded upload evidence. Builds `50`–`54` were not uploaded. Build `52` passed exact CI but reproduced two consecutive provisional `-1005` failures; build `53` passed exact CI in run 30420194567, but live run 30421038307 exhausted two same-connection recovery attempts. Build `54` was superseded before live capture when deeper runtime analysis identified the missing pre-currentness admission gate. Treat `0.5.5` / build `55` as the current update, including `What's New`. No TestFlight distribution is part of this release. A real-iPhone pass remains a separate installed-iOS and ecosystem qualification item. |
