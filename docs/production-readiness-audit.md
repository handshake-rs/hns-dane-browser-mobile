...
| App Store update | Public `1.0.3`; `1.0.4` qualification pending | Apple published `1.0.3` / build `63` on 2026-09-02. Candidate `1.0.4` / build `64` carries the authenticated CNAME and transient-navigation recovery fixes. The still-pending managed default-browser capability and MarketplaceKit app installation entitlement are absent so this update can ship independently. |
