...
non-publishable Rust `1.0.2`, and iOS `1.0.5` / build `66`. Both platform
...
| Area | Status | Finding |
| --- | --- | --- |
| Current source identity | Coordinated dependency cohort; platform candidates independent | Android `1.0.5` / code `57`, iOS `1.0.5` / build `66`, and embedded Rust `1.0.2` use exact HNS releases plus adjacent publication-ready browser-adapter and wallet `0.2.3` sources. [released-dependency-cohort.md](released-dependency-cohort.md) binds that policy. |
...
| App Store update | `1.0.5` / build `66` prepared | The prior `1.0.4` / build `65` submission is historical rather than pending. Current source, metadata, validation, and protected workflow guards target `1.0.5` / build `66`. Signing, exact-source Apple qualification, upload, metadata readback, and intentional submission remain open. The unapproved default-browser and MarketplaceKit app-installation entitlements remain absent. |
