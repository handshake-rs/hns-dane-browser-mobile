...
Current source coordinates Android `1.0.7` / code `59`, embedded
non-publishable Rust `1.0.2`, and iOS `1.0.7` / build `70`. Both platform
coordinates remain unsigned and unuploaded candidates. Earlier
...
Android `1.0.4` / code `56` was committed to Google Play production through
edit `13709111796723000294`, and `generatedApks/56` returned HTTP `200`.
The prior iOS `1.0.4` / build `65` submission replaced withdrawn build `64`;
it is retained below as historical submission evidence, not as the current
candidate.
...
older scoped-loopback compatibility seam. Native Bitcoin, supported name
operations, signed ShakeScape offers, and noncustodial atomic-swap settlement
are exposed behind exact synchronization and native-approval prerequisites.
Website-provider and HNSA/HNSR service-role gates remain false. iOS implements nonblocking read/lifecycle teardown with
...
| Area | Status | Finding |
| --- | --- | --- |
| Current source identity | Coordinated published dependency cohort; platform candidates independent | Android `1.0.7` / code `59`, iOS `1.0.7` / build `70`, and embedded Rust `1.0.2` use exact checksum-bearing HNS `0.4.2`, engine `0.2.5`, and wallet `0.2.5` releases. [released-dependency-cohort.md](released-dependency-cohort.md) binds that policy. |
...
| App Store update | `1.0.7` / build `70` prepared | The prior `1.0.7` / build `69` candidate is superseded after exposing an iOS wallet-directory permission failure. Current source, metadata, validation, and protected workflow guards target `1.0.7` / build `70`. Signing, exact-source Apple qualification, upload, metadata readback, and intentional submission remain open. The unapproved default-browser and MarketplaceKit app-installation entitlements remain absent. |
| Native wallet slice | HNS, Bitcoin, names, and direct ShakeScape active; final release gates pending | The exact pinned controller is connected to native Android/iOS lifecycle, direct HNS and Bitcoin synchronization, receive/QR, guarded sends, activity, name operations, restoration birthday heights, signed offers, durable BTC/HNS atomic-swap execution, and deletion. Websites cannot access it. Current Android two-device exercises cover offer replication and partial forward/reverse settlement; exact terminal reverse-swap completion, final CI, signed products, screenshots, and physical-iPhone evidence remain open. |
| Android runtime opening | Root cause fixed; current toolchain uses upstream implementation | The historical Rust 1.92 build used a locked `libc::flock` shim because standard `File` locks returned `Unsupported` on Android. Rust 1.98 added Android standard-library support in `rust-lang/rust#157038`; current Rust 1.98.1 source uses the standard `File` lock API and removes the custom shim. The exact signed code `47` APK validated the original lock semantics; the current migration remains unbuilt and untested by request. |
