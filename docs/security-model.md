...
The iOS tab surface is metadata-only and count-bounded. It retains no inactive `WKWebView`, JavaScript context, proxy session, or background request. Switching tabs revokes the active coordinator and reconstructs the sole live WebKit view because WebKit exposes no supported clear-history operation; this prevents back-forward or renderer state from crossing tab identities. Page titles and list presentation are length-bounded and control characters are removed before display, and tab metadata is not persisted.
