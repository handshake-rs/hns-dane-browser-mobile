...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PeerState {
    /// Local version sent; remote version required. A remote `verack` may
    /// validly arrive first and is recorded until the version is admitted.
    AwaitingVersion,
...
impl PeerSession {
        ...
        match self.state {
            PeerState::AwaitingVersion => {
                if packet == Packet::Verack {
                    // HSD commonly sends its verack before its version. It is
                    // still not ready until the independently validated
                    // remote version arrives and we send our own verack.
                    self.remote_verack_received = true;
                    Ok(PeerEvent::Ignored(PacketType::Verack))
                } else {
                    self.handle_version(packet, now)
                }
            }
    ...
    fn handle_version(&mut self, packet: Packet, now: u64) -> Result<PeerEvent, PeerError> {
        ...
        self.state = if self.remote_verack_received {
            PeerState::Ready
        } else {
            PeerState::AwaitingVerack
        };
        ...
        loop {
            ...
            match self.session.handle_frame(&frame, now)? {
                PeerEvent::Send(response) => {
                    ...
                    if state_before != PeerState::Ready && self.session.state() == PeerState::Ready
                    {
                        return Ok(PeerEvent::Ready(
                            self.session
                                .metadata()
                                .cloned()
                                .ok_or(PeerError::InternalInvariant)?,
...
mod tests {
    ...
    #[test]
    fn hsd_verack_before_version_completes_wallet_handshake() {
        let now = 1_700_000_000;
        let remote_address = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 14_038);
        let mut remote_version = version([8; 8], now);
        remote_version.services |= SERVICE_BLOOM;
        let inbound_packets = [Packet::Verack, Packet::Version(remote_version)];
        ...
        assert_eq!(metadata.services, SERVICE_NETWORK | SERVICE_BLOOM);
