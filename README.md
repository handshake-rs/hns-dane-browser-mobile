...
Android `1.0.6` / code `58`, iOS `1.0.6` / build `68`, and embedded Rust
...
The next iOS candidate is `1.0.6` / build `68`, carrying the current
...
`68`, and the embedded non-publishable Rust workspace at `1.0.2`. It directly pins
...
- Packages the Rust FFI core for physical Android devices by default:
  `armeabi-v7a` and `arm64-v8a`. CI may explicitly select `x86_64` for its
  Android emulator without adding that ABI to ordinary device APKs.
...
candidate is `1.0.6` / build `68`. Linux validates the Rust, ABI, header, and
